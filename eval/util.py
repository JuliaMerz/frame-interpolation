...
def interpolate_recursively_from_files(
    ...
    Args:
      ...
      bounce: Repeat the start and end frames this many times. Ignores recursivness.

    Yields:
      The interpolated frames (including the inputs).
    """
    print("Confirmation that we're using the from file generator")
    n = len(frames)
    num_frames = (n - 1) * (2 ** (times_to_interpolate) - 1)
    bar = tqdm(total=num_frames, ncols=100, colour="green")
    for j in range(0, bounce):
        yield read_image(frames[0])
    for i in range(1, n):
        yield from _recursive_generator(
        ...
        for j in range(1, bounce):
            yield read_image(frames[i])
